...
object Ksp2Invoker {
  ...
  private fun Map<String, Any?>.optionalStringMap(key: String): Map<String, String> =
    (this[key] as? Map<*, *>)?.map { (mapKey, value) ->
      val typedKey = mapKey as? String ?: error("KSP2 request value '$key' must have String keys")
      val typedValue = value as? String ?: error("KSP2 request value '$key' must have String values")
      typedKey to typedValue
    }?.toMap() ?: emptyMap()
  ...
  private fun buildJvmConfig(request: Map<String, Any?>): KSPJvmConfig =
    KSPJvmConfig
      ...
      .apply {
        ...
        processorOptions = request.optionalStringMap("processorOptions")
      }.build()
}

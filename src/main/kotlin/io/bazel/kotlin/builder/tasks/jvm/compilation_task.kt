...
internal fun JvmCompilationTask.runPlugins(
  ...
  if ((
    ...
    val isKsp = !outputs.generatedKspSrcJar.isNullOrEmpty()
    val apModeExecLabel = if (isKsp) "ksp" else "kapt"

    return context.execute("$apModeExecLabel (${inputs.processorsList.joinToString(", ")})") {
      (
        baseArgs()
          .plus(
            plugins(

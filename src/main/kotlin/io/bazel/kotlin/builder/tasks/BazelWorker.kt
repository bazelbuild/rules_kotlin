      ...
      ByteArrayInputStream(ByteArray(0)).use { emptyIn ->
        ByteArrayOutputStream().use { buffer ->
          PrintStream(buffer).use { ps ->
            ...
            System.setErr(ps)
            while (true) {
              // TODO(rules_kotlin/issues/297): move the directory deletion to a separate process
              //to happen when idle and not block compilation
              WorkingDirectoryContext.newContext().use { wdCtx ->
                val request =
                    WorkerProtocol.WorkRequest.parseDelimitedFrom(realStdIn)
                        ?: return 0
                val exitCode = try {
                  delegate.apply(wdCtx.dir,
                      loadArguments(request.argumentsList, true))
                } catch (e: RuntimeException) {
                  return if (wasInterrupted(e)) INTERUPTED_STATUS
                  else ERROR_STATUS.also {
                    System.err.println(
                        "ERROR: Worker threw uncaught exception with args: ${
                        request.argumentsList.joinToString(" ")
                        }"
                    )
                ...
                realStdOut.flush()
                buffer.reset()
                // TODO(rules_kotlin/issues/297): move gc to a separate process when idle and not
                // block compilation.

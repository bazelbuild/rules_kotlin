 ...
 *
 * Note: This function does NOT add the -Xplugin flag. The plugin jar should be in stubs_plugin_classpath
 * and will be automatically converted to -Xplugin by the plugins() function.
 */
internal fun JvmCompilationTask.annotationProcessingArgs(
  ...
  return CompilationArgs().apply {
    // -Xplugin flag is generated by plugins() function from stubs_plugin_classpath
  ...
  return context.execute("${config.pluginId} ($processorNames)") {
    val args =
      baseArgs()
        .plus(
          plugins(
            // Filter out annotation processing plugin options - they will be added by annotationProcessingArgs()
            options =
              inputs.stubsPluginOptionsList.filterNot { o ->
                o.startsWith(config.pluginId)
              },
            // Don't filter classpath - all plugin jars should be passed to kotlinc
            classpath = inputs.stubsPluginClasspathList,
          ),
        ).plus(
          annotationProcessingArgs(context, config),
...
internal fun JvmCompilationTask.runPlugins(
  ...
  // Plugin jar should be in stubs_plugin_classpath and will be converted to -Xplugin by plugins() function
  // We don't need to pass it separately anymore
  return runAnnotationProcessingPlugin(context, config, compiler)
}

...
class IncrementalCompilationTest {
    ...
    private lateinit var shrunkSnapshotPath: Path

    private val sources = mutableMapOf<String, String>()
    private var previousShrunkSnapshot: String? = null
    ...
    private val component by lazy {
        DaggerKotlinBuilderTestComponent.builder()
            .toolchain(KotlinToolchain.createToolchain(
                ...
                File(Deps.Dep.fromLabel("//kotlin/compiler:kotlin-annotation-processing").singleCompileJar()),
                ...
                File(Deps.Dep.fromLabel("@kotlin_daemon_embeddable//file").singleCompileJar()),
    ...
    fun setUp() {
        ...
        // The actual IC working dir is incrementalBaseDir/ic-caches (see CompilationTask.kt)
        val icWorkingDir = icCachesDir.resolve("ic-caches")
        shrunkSnapshotPath = icWorkingDir.resolve("shrunk-classpath-snapshot.bin")

        sources.clear()
        previousShrunkSnapshot = null
    }
    ...
    private fun compile(isFirstBuild: Boolean = false): CompilationResult {
        ...
        val exitCode = try {
            component.jvmTaskExecutor().execute(context, task)
            0
        } catch (e: CompilationStatusException) {
            e.status
        }

        // After successful build, update the previous shrunk snapshot for next build
        if (exitCode == 0 && Files.exists(shrunkSnapshotPath)) {
            previousShrunkSnapshot = shrunkSnapshotPath.toAbsolutePath().toString()
        }
    ...
    private fun buildTask(forceRecompilation: Boolean): JvmCompilationTask {
        ...
        return JvmCompilationTask.newBuilder().apply {
            ...
            inputsBuilder.apply {
                ...
                // Set previous shrunk snapshot for incremental builds
                previousShrunkSnapshot?.let { previousShrunkClasspathSnapshot = it }
            }
